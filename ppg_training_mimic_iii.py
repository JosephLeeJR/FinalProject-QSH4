...
def create_dataset(tfrecords_dir, tfrecord_basename, win_len=875, batch_size=4, modus='train'):
...
def get_model(architecture, input_shape, UseDerivative=False):
    return {
        ...
        'lstm5' : define_LSTM5(input_shape)
    }[architecture]
...
if __name__ == "__main__":
    ...
    if len(argv) > 1:
        ...
        parser.add_argument('--arch', type=str, default="lstm5",
                            help="neural architecture used for training (alexnet (default), resnet,  slapnicar, lstm, lstm5)")
        parser.add_argument('--lr', type=float, default=0.003, help="initial learning rate (default: 0.003)")
        parser.add_argument('--batch_size', type=int, default=4, help="batch size used for training (default: 32)")
